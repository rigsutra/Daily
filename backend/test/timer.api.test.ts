...
// Timer routes — mirror actual timerService logic with direct Prisma
...
app.post('/api/timer/pause', async (req: any, res: any) => {
  try {
    const userId = getUserId(req);
    const active = await prisma.timerSession.findFirst({ where: { userId, endTime: null } });
...
app.post('/api/timer/stop', async (req: any, res: any) => {
  try {
    const userId = getUserId(req);
    const active = await prisma.timerSession.findFirst({ where: { userId, endTime: null } });
    if (!active) return res.status(400).json({ error: 'No active timer' });
...
app.get('/api/timer/today', async (req: any, res: any) => {
  try {
    const userId = getUserId(req);
...
describe('Timer API', () => {
  ...
  beforeAll(async () => {
    ...
    const res = await request(app)
      ...
      .send({ name: 'Timer API User', email, password: 'password123' });
  ...
  describe('Authentication guard', () => {
    ...
    it('rejects today sessions without token', async () => {
      const res = await request(app).get('/api/timer/today');
      expect(res.status).toBe(401);
    });
  });
  ...
  describe('POST /api/timer/start', () => {
    afterEach(async () => {
      // Clean up any open sessions so each test starts fresh
  ...
  describe('GET /api/timer/today', () => {
    ...
    it('only returns sessions for today (not yesterday)', async () => {
      // Create a session with yesterday's startTime directly via Prisma
  ...
  describe('Full start → pause → resume → stop workflow', () => {
